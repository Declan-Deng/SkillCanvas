...
test("server-renders the SkillCanvas creation experience", async () => {
  ...
  assert.doesNotMatch(html, /SkillCanvas 不会用固定问卷或静态样例冒充 AI 理解/);
  assert.doesNotMatch(html, /从你开始，不从格式开始/);
  assert.match(html, /AI 不懂你/);
...
test("keeps the generator compiler, privacy gate, and prompts wired", async () => {
  ...
  assert.match(route, /Obey evaluation\.allowedMutationTypes exactly/);
  assert.match(route, /Previously rejected repair attempts/);
  ...
  assert.match(page, /tabIndex=\{blueprintExpanded \? 0 : -1\}/);
  ...
  assert.doesNotMatch(page, /第 \{interviewRoundIndex \+ 1\} 轮 · 最多 \{INTERVIEW_ROUND_META\.length\} 轮/);
  ...
  assert.match(page, /className="question-helper-trigger"/);
  assert.match(page, /className="question-helper-tooltip" id=\{helperId\} role="tooltip"/);
  assert.doesNotMatch(page, /<strong>\{question\.label\}<\/strong><small>\{question\.helper\}<\/small>/);
  ...
  assert.match(page, /pipelineIssuesFromEvalFailures\(optimizationSession\.trainingEvidence\)/);
  assert.match(page, /failureAttributions: selectedFailureIssues/);
  assert.match(page, /validatePatchPlan\(\{ plan: attributedPlan, issues: selectedFailureIssues/);
  assert.match(route, /Failure Attribution is a compiler boundary/);
  ...
  assert.match(css, /\.choice-indicator\s*\{[\s\S]*?margin-right:\s*4px/);
  assert.match(css, /\.question-helper:hover \.question-helper-tooltip/);
  assert.match(css, /\.question-helper:focus-within \.question-helper-tooltip/);
  assert.match(css, /\.blueprint-grid\.is-stacked \.blueprint-card:nth-child\(5\)/);
  assert.match(css, /translate\(-3px, -420px\) scale\(1\.018\)/);
  assert.match(css, /\.blueprint-grid::after\s*\{[\s\S]*?box-shadow:\s*0 24px 52px/);
  assert.match(css, /transition:\s*height 260ms var\(--ease-in-out\)/);
